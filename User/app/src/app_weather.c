...
// === 内部函数：解析 JSON (安全增强版) ===
static uint8_t Parse_Seniverse_JSON(char* json_str)
{
    uint8_t ret  = 0;
    ...
    // [优化6] 解析前先清空缓存，防止上一轮的垃圾数据残留
    memset(&s_cache_data, 0, sizeof(s_cache_data));
    ...
    if (cJSON_IsArray(results))
    {
        ...
        if (first)
        {
            ...
            if (loc && now)
            {
                ...
                // [优化4] 使用 sizeof 确保长度安全
                // [优化4] strncpy 不保证封口，必须手动处理
                if (name && name->valuestring)
                {
                    strncpy(s_cache_data.city, name->valuestring, sizeof(s_cache_data.city) - 1);
                    s_cache_data.city[sizeof(s_cache_data.city) - 1] = '\0';
                }

                if (text && text->valuestring)
                {
                    strncpy(
                        s_cache_data.weather, text->valuestring, sizeof(s_cache_data.weather) - 1);
                    s_cache_data.weather[sizeof(s_cache_data.weather) - 1] = '\0';
                }

                // [优化4] 使用 snprintf 防止溢出
                if (temp && temp->valuestring)
                {
                    snprintf(
                        s_cache_data.temp, sizeof(s_cache_data.temp), "%s C", temp->valuestring);
                }

                if (upd && upd->valuestring && strlen(upd->valuestring) > 16)
                {
                    strncpy(s_cache_data.update_time, upd->valuestring + 11, 5);
                    s_cache_data.update_time[5] = '\0';
                }
...
void App_Weather_Task(void)
{
    ...
    case STATE_WAIT_RESP:
        if (UART_RingBuf_ReadLine(&g_esp_uart_handler, s_line_buf, sizeof(s_line_buf), 20) > 0)
        {
            ...
            {
                if (Parse_Seniverse_JSON(s_line_buf))
                {
                    // [优化2] 成功解析后，调用回调前再次判空
                    if (s_data_cb)
                        s_data_cb(&s_cache_data);
