...
- **1**：使用LVGL，让所有图标变得更加生动灵活。
- **2**：添加断电从flash加载缓存文件功能。
- **3**：替换为触摸屏，能实时查询目标城市。
- **4**：引入 FatFs 文件系统，利用 SDIO 接口挂载 TF 卡，将字库和天气图标存入 SD 卡，实现“换肤”功能，解决 Flash 空间瓶颈。
- **5**：接入 AI 大模型 API (如文心一言/ChatGPT)，利用 ESP32 的 HTTP 能力，在屏幕上显示“每日一句”或根据天气的“穿衣建议”。
- **6**：硬件升级为 ESP32-S3 (单芯片方案)，移除 STM32，利用 S3 的强大 AI 算力和 LCD 接口，进一步降低 BOM 成本并提升 UI 帧率。